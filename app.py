...
target_market = st.text_input("Enter your Target Market")
goal = st.text_area("Enter your Business Goal")
budget = st.text_input("Enter your Estimated Budget")
report_type = st.selectbox("Select Report Type", ["Summary", "Full"])

if st.button("Generate Report"):
    with st.spinner("Generating business report..."):
        if report_type == "Summary":
            summary = get_business_feasibility_summary(industry, target_market, goal, budget)
            st.markdown(summary, unsafe_allow_html=True)
        else:
            report = generate_full_report(industry, target_market, goal, budget)
            st.markdown(report, unsafe_allow_html=True)
            
            with st.spinner("Generating visual charts..."):
                trend_summary, trend_df = get_google_trends(industry)
                st.markdown(f"**Google Trends Summary:**\n\n{trend_summary}")
                if trend_df is not None:
                    plot_trends_chart(trend_df, industry)
            
                _, growth_df = get_market_insights(industry)
                if growth_df is not None:
                    plot_growth_bar(growth_df)
                market_summary, hist = get_industry_market_summary(industry)
                st.markdown(f"**Market Behavior (Yahoo Finance - ETF Analysis):**\n\n{market_summary}")
                if hist is not None:
                    plot_etf_price_trend(hist, industry)
                    
                    
            export_to_pdf(report, "business_report.pdf")
            with open("business_report.pdf", "rb") as f:
                st.download_button("Download PDF", f, "business_report.pdf", mime="application/pdf")